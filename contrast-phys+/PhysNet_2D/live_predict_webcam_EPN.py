...
from utils_paths import get_live_runs_subdir, format_label_ratio

# 搜索 PhysNet 2D/results/ 而非 CWD-relative results/
_EPN_RESULTS = os.path.join(_EVAL_DIR, "results")


def _find_latest_epn_run(label_ratio=0):
    """在 PhysNet 2D/results/label_ratio_X/ 下找最新 run"""
    root = os.path.join(_EPN_RESULTS, format_label_ratio(label_ratio))
...
def _find_latest_epn_run_any():
    """遍历所有 label_ratio_*，返回最新 run（按 mtime）"""
    if not os.path.isdir(_EPN_RESULTS):
        return None
    candidates = []
    for name in os.listdir(_EPN_RESULTS):
        if name.startswith("label_ratio_"):
            root = os.path.join(_EPN_RESULTS, name)
            dirs = [d for d in os.listdir(root) if os.path.isdir(os.path.join(root, d)) and d.isdigit()]
            if dirs:
                p = os.path.join(root, max(dirs, key=int))
...
def load_frames_from_video_with_openface(video_path, landmark_path, store_size=128):
    ...
    Returns:
        ...
        timestamps: 每帧时间戳（frame_num / video_fps，用于 fps 估计）
        frame_nums: 与 frames 对齐的 OpenFace/视频帧索引，供 frames_timestamp.csv 对齐 GT
    """
    ...
    for frame_num in range(total_num_frame):
        ...
        frame_nums_out.append(frame_num)
    cap.release()
    return (
        np.array(frames),
        np.array(timestamps),
        np.array(frame_nums_out, dtype=np.int64),
    )
...
def _default_live_output_dir(train_exp_dir):
    """PhysNet 2D/results/live_runs/label_ratio_X/YYYY-MM-DD_HH-MM-SS"""
    subdir = get_live_runs_subdir(train_exp_dir)
    return os.path.join(_EPN_RESULTS, "live_runs", subdir, time.strftime("%Y-%m-%d_%H-%M-%S"))
...
def main():
    ...
    if args.train_exp_dir is None and not args.record_only:
        args.train_exp_dir = _find_latest_epn_run(0) or _find_latest_epn_run_any()
    ...
    if not args.source.isdigit() and os.path.isfile(args.source):
        ...
        if landmark_path:
            ...
            frames, ts, _ = load_frames_from_video_with_openface(
                args.source, landmark_path, store_size=128
            )
    ...
    print("=" * 60)
    print("开始处理视频并计算心率...")
    min_frames = 120 if args.five_sec else fs * 10
    if frames.shape[0] < min_frames:
        raise RuntimeError(f"采集帧数过少，至少需要 {min_frames} 帧({'5s模式' if args.five_sec else '约10秒以上'})")
    ...
    # 【检查点 2.2】max_frames=300 与训练 T=fs*10 一致；5s模式用120帧
    _max_frames = 120 if args.five_sec else fs * 10
    rppg = run_model(model, frames, max_frames=_max_frames)  # 5s模式120帧，默认300帧（10秒@30fps）
    sys.stdout.flush()
    ...
    if args.full_waveform:
        # 完整波形：单窗口推理，覆盖完整视频长度（最多 fs*60 帧）
