...
import sys
import os
# PhysNet 2D/ -> contrast-phys+/ (utils_*, utils_inference)
...
def my_config():
    ...
    label_ratio = 0
    train_exp_num = 1 # 训练 run_id（对应 PhysNet 2D/results/ 下的 run 编号）
    # 绝对路径 -> 读取 PhysNet 2D/results/，与 3D results/ 隔离
    train_exp_dir = os.path.join(_HERE, "results", format_label_ratio(label_ratio), str(train_exp_num))
...
def my_main(_run, e, train_exp_dir, device, time_interval):
    ...
    input_size = config_train.get('input_size', 128)
    model = EfficientPhysNet(config_train['S'], config_train['in_ch'], input_size=input_size).to(device).eval()
