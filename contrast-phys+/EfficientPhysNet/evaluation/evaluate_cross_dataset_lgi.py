# -*- coding: utf-8 -*-
"""
LGI-PPGI-DB S1 零样本跨数据集评估 — 两层结构，per-clip native fps。
...
运行:
  ...
  python EfficientPhysNet/evaluation/evaluate_cross_dataset_lgi.py \\
    results/external_eval/lgi/curriculum/t10/1 \\
    --h5-dir ../datasets/LGI_h5
"""
import argparse
import json
import os
...
def _save_phase1_viz(
    ...
    ax[0].set_title(
        f"LGI S1 | subj={subj} sess={sess} | fs={fs_hz:g} | clip={clip_i + 1}"
    )
    ax[0].legend()
...
def run_evaluation(pred_dir, h5_dir, save_viz=True):
    ...
    print("=" * 68)
    print("LGI S1 cross-dataset eval | Phase1 signal + Phase2 HR")
    print("=" * 68)
    ...
    for npy_name in npy_files:
        ...
        subj = eval_meta.get("subject") or "unknown"
        hr_by_subject.setdefault(subj, [])
        ...
        for i, (rppg, bvp) in enumerate(zip(rppg_list, bvp_list)):
            ...
            if rec2["hr_error"] is not None:
                hr_by_subject.setdefault(subj, []).append(rec2["hr_error"])
    ...
    for subj, errs in hr_by_subject.items():
        if errs:
            phase2_summary["by_subject"][subj] = {
                "n": len(errs),
    ...
    lines = [
        "LGI-PPGI-DB S1 Cross-Dataset Evaluation",
        f"pred_dir: {pred_dir}",
        "",
    ...
    for subj, st in phase2_summary.get("by_subject", {}).items():
        lines.append(f"  MAE [id{subj}]: {ev._fmt(st['mae'])} (n={st['n']})")
    if save_viz:
        lines.append(f"\n  viz: {viz_dir}/")
...
def main():
    ...
    ap.add_argument(
        "--h5-dir",
        default=os.path.join(_PROJECT, "datasets", "LGI_h5"),
